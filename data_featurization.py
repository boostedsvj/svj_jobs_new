...
def metadata_from_path(path):
    ...
    for data_type in ['jetht']:
        if dirname.lower().startswith(data_type):
    ...
    if 'Run' in dirname:
        match = re.search(r'Run(\d+)(\w+)-UL(\d+)', dirname)
        #match = re.search(r'Run(\d+)to([\dInf]+)', dirname)
        metadata['runbin'] = [float(match.group(1)), float(match.group(2))]
...
def modified_preselection(array):
    ...
    Important: ecaldeadcells are not included in here! --> todo list
    """
...
for i, rootfile in enumerate(data.rootfiles):
    try:
        ...
        array = svj.open_data_root(rootfile)
        ...
        if group_data.modified_preselection:
            ...
            array = svj.cr_filter_preselection(array)

        cols = svj.cr_feature_columns(array)
